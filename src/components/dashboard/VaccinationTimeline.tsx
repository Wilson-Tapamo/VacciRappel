...
export default function VaccinationTimeline({ childrenData }: { childrenData: any[] }) {
    const activeChild = childrenData?.[0]; // Default to first child for now
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        {activeChild ? `Suivi de la croissance de ${activeChild.name}` : "Aucun enfant sélectionné"}
                    </p>
                </div>
            ...
            <div className="space-y-8 relative before:absolute before:left-6 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
                {vaccinations.length > 0 ? (
                    vaccinations.map((record: any, idx: number) => (
                        ...
                        >
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 z-10 ${record.status === 'DONE' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-400'
                                }`}>
                                {record.status === 'DONE' ? <Check size={20} /> : <Clock size={20} />}
                            </div>
